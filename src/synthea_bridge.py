...
class SyntheaBridge:
    ...
    def list_sepsis_patients_from_conditions(
        self,
    ) -> List[str]:
    ...
    def load_patient(
        ...
        *fallback_engine* (a :class:`~src.progression.ProgressionEngine`) if
        ...
        if not raw_rows:
            logger.warning(
                "No observations found for requested patient in %s",
                self._obs_path,
            )
            return []
    ...
    def iter_patient(
        ...
        if not readings:
            if fallback_engine is not None and loop:
                ...
                logger.warning(
                    "No readings for patient '%s'; yielding nothing.",
                    patient_id,
                )
            return
...
def _parse_date_to_ms(
    ...
    logger.warning(
        "Cannot parse date '%s'; using current time",
        date_str,
    )
