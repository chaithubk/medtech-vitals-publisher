...
def test_v2_json_schema():
    ...
    for scenario in (
        ...
        assert vital["version"] == SCHEMA_VERSION
        ...
        for f in (
            ...
            ), f"{f} is not numeric (scenario={scenario})"
...
def test_invalid_scenario_raises_error():
    ...
    with pytest.raises(
        ValueError,
        match="Invalid scenario 'unknown_scenario'",
    ):
...
class TestMQTTClient:
    ...
    def test_disconnect_stops_loop_when_started(
        self,
    ):
    ...
    def test_connect_succeeds_first_try(
        ...
        with patch(
            "src.simulator.time.sleep",
            side_effect=_set_connected,
        ):
            result = client.connect()

        assert result is True

    def test_connect_retries_on_exception(
        self,
    ):
